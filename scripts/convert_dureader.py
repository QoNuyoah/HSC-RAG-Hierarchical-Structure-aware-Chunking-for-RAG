# -*- coding: utf-8 -*-
"""Convert local DuReader.zip to HSC-RAG GovernedDocument artifacts."""
...
from app.adapters.dureader_adapter import DuReaderAdapter, write_jsonl, write_queries_csv  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--zip",
        default=str(PROJECT_ROOT / "DuReader.zip"),
        help="Path to local DuReader.zip.",
    )
    parser.add_argument("--source", default="search", choices=["search", "zhidao"])
    parser.add_argument("--split", default="dev", choices=["train", "dev", "test"])
    parser.add_argument(
        "--limit-docs",
        type=int,
        default=50,
        help="Number of QA records to convert. Use a small sample for fast experiments.",
    )
    parser.add_argument(
        "--output-dir",
        default=None,
        help="Output directory. Defaults to data/processed/dureader/{source}_{split}.",
    )
...
def main() -> None:
    ...
    output_dir = (
        ...
        if args.output_dir
        else PROJECT_ROOT / "data" / "processed" / "dureader" / f"{args.source}_{args.split}"
    )
    output_dir.mkdir(parents=True, exist_ok=True)

    adapter = DuReaderAdapter(args.zip)
